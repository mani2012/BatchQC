...
batchqc_corscatter <- function(data.matrix, batch, mod = NULL,quantile_cutoff = 0.25) {
    ...
    suggested_cutoff <- quantile(med_cor, p = quantile_cutoff) - 1.5 * IQR(med_cor)
...
#' Produce Median Correlation list
#' 
#' @param data.matrix Given data or simulated data from rnaseq_sim()
#' @param batch Batch covariate 
#' @param mod Model matrix for outcome of interest and other covariates 
...
#' pdata <- data.frame(batch, condition)
#' modmatrix = model.matrix(~as.factor(condition), data=pdata)
#' batchqc_corscatter_extract(data.matrix, batch, mod=modmatrix, quantile_cutoff = 0.25)
batchqc_corscatter_extract <- function(data.matrix, batch, mod = NULL, quantile_cutoff = 0.25) {
    ...
    # Create a list to store the median correlation values and suggested cutoff
    result <- list(median_correlation = med_cor, cutoff = suggested_cutoff)
