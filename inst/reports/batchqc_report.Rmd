...
batchqc_ev <- batchqc_explained_variation(shinyInput$data, 
...
if (ncond > 1)  {
    ...
    fit <- lmFit(shinyInput$data, mod_full)
    fit2 <- eBayes(fit)
...
batchqc_circosplot(shinyInput$data, shinyInput$batch, "complete")
```
...
panderOptions("table.split.table", 240)  ## table split at 240 (default 80) characters in a line
...
cat(paste("Note: Overall p-value is calculated for the",
...
kstest <- combatPlot(lcounts, batch=shinyInput$batch, mod=mod)
if (!is.null(kstest))  {
    shinyInput <- getShinyInput()
...
if (ncond <= 1)  {
    modmatrix = matrix(rep(1, ncol(dat)), ncol = 1)
} else  {
